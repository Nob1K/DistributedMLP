...
// Scale a matrix by a scalar value via DIVISION
...
// curr is weights after training, oring is weights before training
// Stores result to curr after operation
...
bool mlp::init_training(string fname, int _k, int _h)
{
    // sets X and X_labels
    // also sets n and d
...
bool mlp::init_training(string fname, vector<vector<double>>& _V, vector<vector<double>>& _W)
{
    // sets X and X_labels
    // also sets n and d
    if(!read_data(X, X_labels, fname, true))
...
// train the model assuming defined weights
// fname = training data file name
// k = number of output units
// h = number of hidden units
// eta = learning rate (good with 0.00001)
// ASSUMES weights have been initialized
// ASSUMES data has been read in
// Returns -1 on error
...
// ASSUMES dV and dW are sized properly
